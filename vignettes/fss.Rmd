...
Most of the data associated with records missing gcs_using is also missing.
For simplicity we will omit these records.


```{r label = "subset-via-gcsusing"}
...
status.
`r  n_perc(pedalfast$fsstotal >= 8)  `
had at least "mildly abnormal" functional status and
`r  n_perc(pedalfast$fsstotal >= 10)  `
